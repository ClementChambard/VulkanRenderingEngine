...
LveModel::LveModel(LveDevice& device, Builder const& builder) :
    lveDevice(device) {
...
void LveModel::createVertexBuffers(std::vector<Vertex> const& vertices) {
    ...
    assert(vertexCount >= 3 && "Vertex count must be min 3");
    ...
    void* data;
    vkMapMemory(lveDevice.device(), stagingBufferMemory, 0, bufferSize,
                0, &data);
...
void LveModel::createIndexBuffers(std::vector<uint32_t> const& indices) {
...
void LveModel::Builder::loadModel(std::string const& filepath) {
